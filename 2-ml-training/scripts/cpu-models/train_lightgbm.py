...
def validate_data(df, name):
    ...
    if 'rps' in df.columns:
        required_columns.append('rps') # Include rps if present
...
def create_features(data):
    ...
    # Create lag features for pod_count
    for lag in [1, 2, 3, 4]:
        result[f'pod_count_lag_{lag}'] = result['pod_count'].shift(lag)
        
    # Create RPS features if available
    if 'rps' in result.columns:
        for lag in [1, 2, 3, 4]:
            result[f'rps_lag_{lag}'] = result['rps'].shift(lag)
        for window in [3, 6, 12]:
            result[f'rps_rolling_mean_{window}'] = result['rps'].rolling(window=window).mean()
            result[f'rps_rolling_std_{window}'] = result['rps'].rolling(window=window).std()
    
    # Create rolling mean features for pod_count
    for window in [3, 6, 12]:
...
def main():
    try:
        ...
        output_dir = os.path.join('./train/models/lightgbm_model/runs', args.run_id)
        ...
        params = {
            ...
            'max_depth': 6,
            'min_data_in_leaf': 20,
            'force_col_wise': True  # More efficient for small datasets
        }
        ...
        # Train model with proper early stopping callback
        with open(os.devnull, 'w') as f, redirect_stdout(f):
            model = lgb.train(
                ...
                callbacks=[
                    ...
                    lgb.log_evaluation(period=-1)  # Suppress logging output
                ]
            )
        ...
        plt.title('LightGBM Model - Pod Count Prediction')
        plt.xlabel('Time')
